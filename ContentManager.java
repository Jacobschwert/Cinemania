...
import java.sql.Connection;
 ...
 import java.sql.SQLException;
  ...
  public class ContentManager {
     ...
     {
        int contentListID = generateContentListID();
         //Needs to be assigned a unique id;
         contentLists.add(new ContentList());
         int newlistIndex = contentLists.size()-1;
         contentLists.get(newlistIndex).setContentListID(contentListID);
         queryString = "INSERT INTO contentList(contentListID, authorID, listName, listDescription, contentIDs) VALUES('" + contentListID + "', " + user.getAccountNumber() + ", " + contentLists.get(newlistIndex).getContentListName() + ", " + contentLists.get(newlistIndex).getContentListDescription() + ", " + contentLists.get(newlistIndex).getContentIDs() + ");";
         try {
     ...
     public int generateContentListID() {
        Random rand = new Random();
        Boolean moveOn = false;
        int number = 0;
        while (moveOn == false){ //Generate ID
            number = rand.nextInt(88888) + 11111; //This gives a range of 11111 - 99999
            queryString = "SELECT contentListID FROM account WHERE contentListID = " + number + ";" ;
            ResultSet rs;
            try{
